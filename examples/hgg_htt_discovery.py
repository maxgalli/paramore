...
class ConstrainedParameter(evm.Parameter):
    ...
    @property
    def prior(self):
        return evm.pdf.Normal(
            mean=jnp.array(0.0), width=jnp.array(self.constraint_width)
        )
...
if __name__ == "__main__":
    ...
    if run_hgg:
        ...
        globalscale = ConstrainedParameter(
            value=0.0,
            name="CMS_hgg_globalscale",
            lower=-0.018868,
            upper=0.018868,
            transform=minuit_transform,
            constraint_width=0.004717,
        )
        smear = ConstrainedParameter(
            value=0.0,
            name="CMS_hgg_nuissancedeltasmearcat0",
            lower=-0.006176,
            upper=0.006176,
            transform=minuit_transform,
            constraint_width=0.001544,
        )
        r = evm.Parameter(
            value=1.0, name="r", lower=0.0, upper=10.0, transform=minuit_transform
        )
        bkg_norm = evm.Parameter(
            value=233.0,
            name="bkg_norm",
        ...
        g1 = pm.Gaussian(
            mu=mu1, sigma=smeared_sigma(SIGMA1_NOM), lower=MASS_LO, upper=MASS_HI
        )
        g2 = pm.Gaussian(
            mu=mu2, sigma=smeared_sigma(SIGMA2_NOM), lower=MASS_LO, upper=MASS_HI
        )
        gw = pm.Gaussian(
            mu=muw, sigma=smeared_sigma(SIGMAW_NOM), lower=MASS_LO, upper=MASS_HI
        )
        ...
        n_ggh = jnp.squeeze(
            (lumi_mod @ sj_mod @ nid_mod)(jnp.array(r.get_value() * N_GGH_BASE))
        )
        ...
        ax.plot(
            ...
            "r-",
            lw=1.5,
            label=rf"ggH ($\mu=1$, {float(n_ggh):.2f} evt)",
        )
        ax.set_xlim(100, 180)
        ax.set_ylim(bottom=0)
        ax.set_xlabel(r"$m_{\gamma\gamma}$ [GeV]")
        ...
        bkg_pdf = pm.BernsteinPolynomial(
            ...
            ),  # square the coefficients as done in Combine
            lower=MASS_LO,
            upper=MASS_HI,
        )
        ...
        ax.plot(
            ...
            "b-",
            lw=1.5,
            label=f"Background (Bernstein, {float(bkg_norm.get_value()):.0f} evt)",
        )
        ...
        def loss_hgg(dynamic, args):
            ...
            # Poisson NLL
            log_likelihood = (
                evm.pdf.PoissonContinuous(lamb=expectation).log_prob(obs_histo).sum()
            )
        ...
        def param_uncertainty(selector):
            ...
            def value_fn(flat_params):
                params_unwrapped_local = nnx.merge(
                    graphdef, unravel(flat_params), static, copy=True
                )
    ...
    if run_htt:
        ...
        print(f"r = {float(fitted_params.r.get_value()):.4f} ± {float(r_sigma):.4f}")
        print(
            f"lumi_8TeV = {float(fitted_params.lumi_8TeV.get_value()):.4f} ± {float(lumi_8TeV_sigma):.4f}"
        )
        print(
            f"scale_e = {float(fitted_params.scale_e.get_value()):.4f} ± {float(scale_e_sigma):.4f}"
        )
